...
export default function Drivers(){
  ...
  const handleExportSummary = () => {
    ...
    const rows = list.map((d) => ({
      id: d?.id || d?.uid || '',
      driver: d?.display_name || d?.name || d?.full_name || d?.email || '',
      email: d?.email || '',
      region: d?.state || d?.region || d?.home_state || '',
      managed_by: d?.managed_by_name || d?.managed_by || d?.sub_admin_name || '',
      role: d?.role || '',
    ...
    downloadCsv(`drivers_summary_${new Date().toISOString().slice(0, 10)}.csv`, rows);
  };
