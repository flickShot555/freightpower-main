...
export default function AdminAnalytics() {
  ...
  useEffect(() => {
    ...
    const fetchAiInsights = async () => {
      ...
      try {
        ...
        console.error('Failed to fetch super-admin AI insights:', error);
  ...
  const aiInsightCards = useMemo(() => {
    ...
    const normalized = rows
      .map((row, idx) => ({
        id: String(row?.id || `super_admin_ai_insight_${idx}`),
    ...
    return [
      {
        id: 'super_admin_ai_fallback_docs',
      ...
      {
        id: 'super_admin_ai_fallback_support',
        title: 'Support Queue',
      ...
      {
        id: 'super_admin_ai_fallback_tracking',
  ...
  const handleExport = () => {
    ...
    downloadJson(`admin_analytics_${range}_${new Date().toISOString().slice(0, 10)}.json`, snapshot);
  };

  const runAiInsightAction = (target) => {
    const actionTarget = String(target || '').trim();
    if (!actionTarget) return;
    window.location.href = `/super-admin/dashboard?nav=${encodeURIComponent(actionTarget)}`;
  };
    ...
    <div className="adm-analytics-root">
      <header className="fp-header adm-analytics-header">
        ...
        <div className="adm-analytics-controls">
          ...
          <button className="btn small ghost-cd" onClick={handleExport}>Export</button>
        </div>
      </header>
