...
def main() -> int:
    component_files = [
        Path("src/components/driver/HiringOnboarding.jsx"),
        Path("src/components/driver/OnboardingCoach.jsx"),
        Path("src/components/driver/AiHub.jsx"),
        Path("src/components/driver/DocumentVault.jsx"),
        Path("src/components/driver/DriverDashboard.jsx"),
    ...
    for lang in LANGS:
        missing_by_lang[lang] = sorted([k for k in referenced if k not in dict_keys_by_lang[lang]])

    missing_total = sum(len(v) for v in missing_by_lang.values())

    print("Component files:", len(component_files))
    print("Keys referenced (unique):", len(referenced))
    for lang in LANGS:
        print(f"Missing in {lang}:", len(missing_by_lang[lang]))

    if missing_total:
        print("\nFirst 150 missing per language:")
        for lang in LANGS:
            missing = missing_by_lang[lang]
            if not missing:
                continue
            print(f"\n[{lang}]")
            for k in missing[:150]:
                print("-", k)
        return 1
